...
## Repeated-Section Safety

- Treat each country or regional section as an independent scope, even when several sections contain identical hull blocks.
- Anchor every insertion to both the section key and the hull key. Never apply a replacement using only a repeated icon line such as `Queen.png` or `Admiral.png`.
- Before editing, count the expected placements as `number of target sections * number of hull tiers in the chain` and use that count for validation.
- After editing, verify that every expected section and hull contains exactly one new entry, that no earlier or unrelated hull contains it, and that no duplicate entry was created.
- If a bulk edit reports a partial, corrected, or ambiguous application, stop and inspect counts and section boundaries before making another edit.
...
9. Validate the completed chain before reporting success: check the exact occurrence count, inspect the first and last target sections, confirm the icon remains at the same relative position in every pool, and confirm no duplicate or misplaced entries exist.
